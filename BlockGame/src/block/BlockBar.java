...
public class BlockBar extends Block {
	 ...
	 */
	public BlockBar(int x, int y, Color color, List<Point<Integer>> points) {
		super(x, y, new ArrayList<Box>());
	 ...
	 * @param other
	 */
	public BlockBar(BlockBar other) {
		super(other);
	...
	protected List<Block> getErased(Set<Integer> iBoxes) {
		...
		} else if (iBoxes.containsAll(Set.of(0, 1, 3))) {
			ret = BlockCreater.create1(x, y, boxes, 2);
		...
		} else if (iBoxes.containsAll(Set.of(0, 3))) {
			ret = BlockCreater.create2(x, y, boxes, 1, 2);

		} else if (iBoxes.containsAll(Set.of(1, 2))) {
			ret = BlockCreater.create1_1(x, y, boxes, 0, 3);

		} else if (iBoxes.containsAll(Set.of(1, 3))) {
			ret = BlockCreater.create1_1(x, y, boxes, 0, 2);

		} else if (iBoxes.containsAll(Set.of(2, 3))) {
			ret = BlockCreater.create1_1(x, y, boxes, 0, 1);
